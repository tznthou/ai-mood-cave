...
-   **<i class="fas fa-chart-line"></i> 心情記錄**：點擊這個按鈕，可以看到你的心情記錄和趨勢圖表。
-   **<i class="fas fa-cog"></i> 設定**：點擊這裡，可以管理 APP 的設定，包括深色模式、數據保留期限等。
...
#### 📅 數據保留期限 **（新功能！）**
你現在可以自由選擇心情記錄要保存多久：
- **24小時（預設）**：傳統的「用後即焚」模式，記錄會在一天後自動刪除
- **一週（7天）**：保存一週的記錄，可以觀察短期的心情變化
- **一個月（30天）**：保存一個月的記錄，適合進行心情趨勢分析
- **三個月（90天）**：保存三個月的記錄，最適合長期心情追蹤

💡 **使用建議**：如果你想要獲得更好的心情分析效果，建議選擇「一個月（30天）」或更長的保留期限，這樣 AI 可以根據更多的歷史資料提供更準確的建議。

#### 🎨 外觀設定
- **深色模式**：開啟後，介面會變成深色主題，適合夜間使用或保護眼睛

#### 🔒 隱私設定
- **自動刪除過期記錄**：根據你選擇的保留期限，自動清除舊的心情記錄
- **僅本地儲存**：確保所有資料只存在你的設備上，不會上傳到網路

#### 🔔 通知設定
- **每日心情記錄提醒**：開啟後會提醒你定期記錄心情（功能準備中）
...
-   **自動刪除機制**：你可以自由選擇記錄保留期限（24小時～3個月），過期的記錄會自動被永久刪除，不留痕跡。
-   **隨時清除**：你也可以在「設定」中，隨時手動清除所有資料。

無論你選擇哪種保留期限，你都可以安心地在這裡傾訴內心想法。
...
**Q: 我之前的日記不見了，為什麼？**
A: 為了保護你的隱私，系統會自動刪除超過保留期限的記錄。預設是 24 小時自動刪除，但你可以在「設定」中調整保留期限（24小時、一週、一個月或三個月）。

**Q: 如何更改記錄保留期限？**
A: 很簡單！點擊右上角的 <i class="fas fa-cog"></i> 設定按鈕，在「數據保留期限」下拉選單中選擇你想要的期限即可，設定會立即生效。
